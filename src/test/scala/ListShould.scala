package tests {
  ...
  class ListShould extends FunSuite {
    ...
    test("Get a new list built with all elements but the last of an existing list") {
      val source = List2[Int](1, 2, 3, 4, 5)
      val expected = List2[Int](1, 2, 3, 4)

      val rebuilt = List2.init(source)
      assert(expected == rebuilt)
    }

    test("Can append a list to another") {
      val source = List2[Int](1, 2, 3, 4, 5)
      val appended = List2.append(source, List2[Int](6, 7))

      assert(appended == List2[Int](1, 2, 3, 4, 5, 6, 7))
    }
  }
}